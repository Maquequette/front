...
export default function Comment({ comment }: IComment) {
  ...
  const moreReply = useCallback(async () => {
    const results = await getCommentsFrom({ parent: comment.id });
      ...
      <div className="comment__header">
        <div className="comment__header__infos">
          <Like
            showNumber={true}
            id={comment.id}
            isAlreadyLiked={comment.isLiked}
          ...
          <span>
            {t("Posted on") +
              " " +
              dateToFormat(new Date(comment.createdAt), "dd/mm/yyyy") +
              " " +
              t("at") +
              " " +
              dateToFormat(new Date(comment.createdAt), "hh:mm")}
          </span>
        </div>
        <ProfileButton
      ...
      <div className="comment__body">
        ...
        {children.length === 0 && comment.commentsCount > 0 && (
          <button className="comment__body__count" onClick={moreReply}>
            <span>{"+ " + comment.commentsCount + " " + t("more messages")}</span>
          </button>
        )}
        {children.length !== 0 && (
